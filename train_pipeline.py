...
class PHM_STGNN_Model(nn.Module):
    ...
    集成前面四个阶段的完整端到端预测模型
    """
    def __init__(self, num_nodes=8, c_in=1, d_model=256, cond_dim=2, num_classes=3):
        ...
        # 注意：我们在主干内部拦截 Transformer 之前的特征注入提示
        self.st_backbone = SpatioTemporalBackbone(
            num_nodes=num_nodes, c_in=c_in, c_cnn=32, d_gcn=64, d_model=d_model
        )
        # 提示工程模块
        self.condition_prompt = ConditionPrompt(cond_dim=cond_dim, d_model=d_model)
        # 阶段四：多任务预测头
        self.mt_head = MultiTaskHead(d_model=d_model, hidden_dim=128, num_classes=num_classes)
